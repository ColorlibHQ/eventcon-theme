...
// Final Class
final class Eventcon {

	// Theme Version
	private $eventcon_version = '1.0';
	...
	private function setup() {
		...
		$enqueu          = new eventcon_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->eventcon_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'eventcon_content_width', 751 );
		...
		load_theme_textdomain( 'eventcon', EVENTCON_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'eventcon_site_logo_117x45', 117, 45, true );

		// Performer thumb size
		add_image_size( 'eventcon_performer_img_362x450', 362, 450, true );

		// About thumb size
		add_image_size( 'eventcon_performer_img_686x506', 686, 506, true );

		// Program thumb size
		add_image_size( 'eventcon_program_img_264x280', 264, 280, true );
		
		// Single blog post image size
		add_image_size( 'eventcon_single_blog_750x375', 750, 375, true );
		add_image_size( 'eventcon_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu'	=> esc_html__( 'Primary Menu', 'eventcon' ),
			'services'		=> esc_html__( 'Services', 'eventcon' ),
			'useful-links'	=> esc_html__( 'Useful Links', 'eventcon' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = EVENTCON_DIR_CSS_URI;
		$jsPath  = EVENTCON_DIR_JS_URI;
		...
		$scripts = array(
			'style'   => array(
				...
				array(
					...
					'dependency' => array(),
					'version'    => $this->eventcon_version,
				),
				array(
					'handler' => 'eventcon-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'eventcon' ) ) {
			
			$font_families = array(
				'Anton|Monoton|Muli:400,400i,500,500i,600',
			);
	...
	private function customizer_init(){
		...
		// Instantiate eventcon theme customizer
		$eventcon_theme_customizer = new eventcon_theme_customizer();
	}
	...
	public function epsilon_quickie(){
			...
			'links' => array(
				array(
					'link_to'   => 'eventcon_options_panel',
	...
	public function set_elementor_flag(){
		update_option( 'eventcon_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'eventcon-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'eventcon_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'eventcon-elementor-default-style',  EVENTCON_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->eventcon_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'eventcon_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'eventcon-elementor-notice', EVENTCON_DIR_JS_URI.'eventcon-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'eventcon-elementor-notice',
				'eventconElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'eventcon-elementor-notice-nonce' ),
				)
			);
		}
