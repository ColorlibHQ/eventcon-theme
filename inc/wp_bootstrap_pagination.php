...
function eventcon_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'eventcon' ),
        'next_string'     => esc_html__( '&raquo;', 'eventcon' ),
        'before_output'   => '<div class="eventcon-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'eventcon_pagination_defaults', $defaults )
    );
