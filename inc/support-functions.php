...
function eventcon_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'eventcon' ) . '</span></li>';
...
// Blog categories
function eventcon_featured_post_cat(){
...
// social media
if ( ! function_exists( 'eventcon_social' ) ) {
	function eventcon_social( $args = array() ) {
		...
		$url = eventcon_opt( 'eventcon_social_url' );
...
// header cart count
function eventcon_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( EVENTCON_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'eventcon' ); ?>">
...
function eventcon_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'eventcon_contact7_form_content', 10, 2 );